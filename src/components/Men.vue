<template>
  <div class="container">
    ...
    <div class="row mt-5" id="1">
      <div class="col-md-4" v-for="menProduct in menProducts" :key="menProduct.id" >
        <div class="card mb-4 product">
          ...
          />
          <div class="card-body">
            <h4 class="text-uppercase text-muted">{{ menProduct.category }}</h4>
            <p class="card-title">{{ menProduct.title }}</p>
            <div class="d-flex justify-content-between">
              <p class="fs-5 fw-bolder price">{{ menProduct.price }} Euro</p>
              <b-button @click="displayModal(menProduct)">Buy Now</b-button>
            </div>
          </div>
        </div>
      </div>
              ...
              <div class="col-md-6">
                <div id="outputCardDetails">
                  <div class="card mb-5 product" id="3">
                    <img
                      class="card-img-top productImg"
                      :src="menProductCard.image"
                      :alt="menProductCard.title"
                      style="height: auto"
                    />
                  </div>
                </div>
              ...
              <div class="col-md-6">
                <div id="outputCardBody">
                  <h4 class="text-uppercase text-muted card-text">
                   {{ menProductCard.category }}
                  </h4>
                  <p class="card-title"> {{ menProductCard.title }}</p>
                  <p class="description" style="display: block">
                    {{ menProductCard.description}}
                  </p>
                  <div class="d-flex justify-content-between">
                    <p class="fs-5 fw-bolder price"> {{ menProductCard.price }} Euro</p>
                    </div>
                </div>
                <form id="formModale">
                  <div class="mb-3 row">
...
export default {
  ...
  methods: {
    displayModal(menProduct) {
      this.menProductCard = menProduct;
      this.$refs["men-modal"].show();
    },
  },
  created() {
    db.collection("products")
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            ...
            if(MenCategory === "men's clothing"){
            this.menProducts.push(doc.data());
              }
