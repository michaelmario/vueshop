      ...
      <div class="col-md-4" v-for="electProduct in electProducts" :key="electProduct.id">
        <div class="card mb-4 product">
          <img
            ...
            :src="electProduct.image"
            :alt="electProduct.title"
          />
          <div class="card-body">
            <h4 class="text-uppercase text-muted">{{ electProduct.category }}</h4>
              ...
              <div class="col-md-6">
                <div id="outputCardDetails">
                  <div class="card mb-5 product" id="3">
                    <img
                      class="card-img-top productImg"
                      :src="electProductCard.image"
                      :alt="electProductCard.title"
                      style="height: auto"
                    />
                  </div>
                </div>
              ...
              <div class="col-md-6">
                <div id="outputCardBody">
                  <h4 class="text-uppercase text-muted card-text">
                   {{ electProductCard.category }}
                  </h4>
                  <p class="card-title"> {{ electProductCard.title }}</p>
                  <p class="description" style="display: block">
                    {{ electProductCard.description}}
                  </p>
                  <div class="d-flex justify-content-between">
                    <p class="fs-5 fw-bolder price"> {{ electProductCard.price }} Euro</p>
                    </div>
                </div>
                <form id="formModale">
                  <div class="mb-3 row">
...
export default {
  ...
  methods: {
    displayModal(electProduct) {
      this.electProductCard = electProduct;
      this.$refs["elect-modal"].show();
    },
  },
  created() {
    db.collection("products")
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            ...
            this.electProducts.push(doc.data());
              }
